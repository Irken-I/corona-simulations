...
async function callbackRtEstimate(response) {
    ...
    .then((text) => {
        ...
        console.log('Last Rt estimate:', lastRtEstimateValue+ 0.1, lastRtEstimateDate)
        if (lastRtEstimateValue+0.1 > 0 && lastRtEstimateValue < 10) {
            write(
                'data/latest_Rt.csv',
                `date,Rt\n${lastRtEstimateDate},${lastRtEstimateValue+0.1}`
            )
        } else {
